# Z-scores for scores, log
# Import setup
freq_setup <- read_csv_file("setupData/freq_log_setup.csv")

# Freq name save file suffix
common_word_freq <- "20240513-log-freq"
common_word_vio <- "20240513-log-violin"
...
create_frequency_plot <- function(software, data, upperY, save_to_file = FALSE) {
  ...
  freq <- ggplot() +
    ...
    coord_cartesian(xlim = c(-3, 3), ylim = c(0, upperY))  # Use IQR-based limits
    # 
      ...
      for(clade in unique(data$Clade)) {
        ...
        freq <- freq +
          geom_freqpoly(data = filtered_data, aes(x = Zscore, y = ..count../sum(..count..), color = ControlLabel, linetype = Clade),
                        breaks = breaks, linewidth = 0.7, alpha = 0.8)
      }
    }
...
for (software in software_names) {
  ...
  all_scores <- score_data %>% pull(Score)
  ...
  if(global_min <= 0){
    epsilon = 0.01 * abs(global_max)
    score_data <- score_data %>% mutate(adjScore = Score + abs(global_min) + epsilon)
  }else{
    score_data <- score_data %>% mutate(adjScore = Score)
  }
  ...
  for (clade in names(combined_tool_results_list[[software]])) {
    ...
    clade_data <- score_data %>% filter(Clade == clade) %>% mutate(LogScore = log10(adjScore))
    #clade_data <- score_data %>% filter(Clade == clade)
    
    # Separate the scores based on the new labels
    off_scores <- clade_data %>% filter(ControlLabel == "Intergenic") %>% pull(LogScore)
    shuf_scores <- clade_data %>% filter(ControlLabel == "Shuffled") %>% pull(LogScore)
    ...
    # Calculate zscore
    clade_data <- clade_data %>% mutate(Zscore = (LogScore - pooled_neg_mean)/pooled_neg_sd)
